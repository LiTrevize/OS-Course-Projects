    ...
    public int request(int pid, int size, char flag) {
        ...
        for (Proc p : list) {
            if (size(p) >= size) {
                int base = p.base;
                p.base += size;
                mem.add(mem.indexOf(p), new Proc(pid, base));
                return 0;
            }
        }
